<template>
	<div>
		...
		<div style="display: inline-block;width: 250px;margin: 10px 0 0 20px;">
			<h1>船队信息</h1>
			<el-table :data="reportList" :row-class-name="colors" @row-click="produceReportfind">
		...
		<div style="display: inline-block;width: 800px;margin: 10px 0 0 20px;
			...
			<div style="font-size: 14px;color: #606266;">
				...
				<el-row style='margin-top: 20px;'>
				  <el-col :span="6" >
					   <el-select  placeholder="请选择流程"  v-model='flowid' style='width: 150px;' @change="baseFlowfind()">
				...
				<el-row style='margin-top: 20px;'>
				  ...
				  <el-col :span="6">
					<el-button type='primary'  @click="stars()" :disabled="status!='未完成'">开始作业</el-button>
				  </el-col>
				  <el-col :span="6">			
			...
			// 后端网址
				...
				// 后端网址
					...
					this.$axios.post(url,this.$qs.stringify(data),{
						...
						var row=res.data;
						this.dljname=row.dljname;
						this.pdjname=row.pdjname;
						this.zcjname=row.zcjname;
				...
				// 后端网址
					var url = this.baseUrl+"/produceReport/load"   //调用后端查询方法
					// 传递给后端的数据
					...
					this.$axios.post(url,this.$qs.stringify(data),{
						...
						if(row.startjobtime==null){
							this.status="未完成"
						}else if(row.startjobtime!=null&&row.completetime==null){
				...
				//调用后端查询方法
				// 传递给后端的数据
				var data={reportid:this.reportids,startjobtime:this.startjobtime}
				this.$axios.post(url,this.$qs.stringify(data),{
					headers: {
						'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'
					}
				...
				this.time()
					var url = this.baseUrl+"/produceReport/completeTask"   //调用后端查询方法
				// 传递给后端的数据
// 				alert(this.reportids)
// 				alert(this.completetime)
// 				alert(this.flowid)
				var data={reportid:this.reportids,completetime:this.completetime}
				this.$axios.post(url,this.$qs.stringify(data),{
					headers: {
						'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'
					}
